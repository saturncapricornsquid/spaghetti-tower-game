...
function initGame() {
  ...
  updateHud();
  statusEl.innerText =
    "Build phase: place spaghetti or connector marshmallows. Pieces stay fixed while building, then unlock for the final test.";
  startTimer();
}
...
function addGlue(connector, stick, localXOnStick) {
  ...
  const c = Constraint.create({
    ...
    length: currentDistance,   // prevents violent snapping
    stiffness: 0.35,
    damping: 0.15,
    render: { visible: false }
  });
...
/* Unlock all build pieces for final test */
